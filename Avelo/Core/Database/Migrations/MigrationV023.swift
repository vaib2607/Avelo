...
public struct MigrationV023: Migration {
    public let version: SchemaVersion = .v23
    public let description = "Widen audit action CHECK constraint for AVL-P0-034 coverage (FY, banking, orders, groups, BOMs, templates, exports)"
    ...
    public func up(_ db: SQLiteDatabase) throws {
        ...
        try db.execute("ALTER TABLE avelo_audit_events RENAME TO avelo_audit_events_v22;")
        try db.execute(
            """
        ...
        // Straight copy: widening the CHECK constraint doesn't change any
        // stored value, so the existing chain_hmac/sequence_number values
        // remain valid without recomputation (unlike MigrationV012, which
        // was backfilling those columns for the first time).
        try db.execute(
            """
            ...
            FROM avelo_audit_events_v22
            ORDER BY company_id ASC, sequence_number ASC;
            """
        )
        try db.execute("DROP TABLE avelo_audit_events_v22;")
