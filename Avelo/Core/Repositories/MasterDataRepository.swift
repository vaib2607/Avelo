...
=======
    ...
    public func listCostCategories(companyId: Company.ID) throws -> [CostCategory] {
        try db.query("SELECT id, company_id, code, name, is_active, created_at FROM avelo_cost_categories WHERE company_id = ? ORDER BY code", bind: [.text(companyId.uuidString)]) {
            CostCategory(id: try UUIDParsing.required($0.text("id"), field: "avelo_cost_categories.id"), companyId: try UUIDParsing.required($0.text("company_id"), field: "avelo_cost_categories.company_id"), code: $0.text("code"), name: $0.text("name"), isActive: $0.bool("is_active"), createdAt: $0.timestamp("created_at"))
        }
>>>>>>> origin/main
    }
}
