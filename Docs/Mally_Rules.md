...
- The SQLite file under `Application Support/Mally/Companies/<uuid>.sqlite` is the only authoritative store.
...
- Each company is one `.sqlite` file at `Application Support/Mally/Companies/<uuid>.sqlite`.
- A small `registry.sqlite` in `Application Support/Mally/` lists known companies for the picker. It never holds financial data.
...
- All type, method, table, and column names are defined in `Mally_Naming_Freeze.md`. Renaming requires updating the freeze document first. Ad-hoc renames in later passes are rejected.

## R-15. No placeholders, no TODOs, no `fatalError("Not implemented")`

- Every file is complete. Every public method has a body.
- If a feature is genuinely out of scope, it is documented in `Mally_Module_Checklist.md` as "deferred to Phase N" and the corresponding public API is either absent or returns a typed `AppError.featureUnavailable`.
