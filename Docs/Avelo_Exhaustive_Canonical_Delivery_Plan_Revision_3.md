...
| AVL-P2-013 Ctrl+I/PgUp/PgDn | ~~Implemented / automated proof~~; manual acceptance remaining | Selection movement and boundary clamping fully proven (`testSelectNextAndPreviousMoveThroughLoadedPageWithoutTouchingFilterState`, `testSelectNextAtLastRowIsANoOp`). ⌘I (originally Ctrl+I, converted to native Mac 2026-07-22) structurally cannot mutate list/filter/selection state — its handler never references the list view model, only routes through `AppActionRegistry.perform` to open a fresh journal sheet. Dirty-state cooperation is inherent via the existing focus-scoped `.onKeyPress` (fires only when the voucher table owns focus, never during text entry). Keyboard/VoiceOver acceptance open. |
...
**PARTIAL — current implementation evidence:** all five originally
Tally-style (Ctrl/Alt) shortcuts were converted to native Mac Cmd-based
chords (2026-07-22): Ctrl+R→⇧⌘R (narration recall), Ctrl+V→⌘E (item-invoice
toggle), Ctrl+I→⌘I (new journal from table), Alt+2→⌘D (duplicate voucher),
Alt+C→⌘N (create account). Behavioral scoping is unchanged by the remap:
⌘N remains limited to the focused eligible `AccountPicker`; ⌘E is limited
to a fresh eligible Sales/Purchase draft (never recovery or text editing)
and ⇧⌘R is Narration only; ⌘I, ⌘D, PgUp, and PgDn
are registered only on the focused voucher table, replacing hidden global
...
mechanism ⌘E/⇧⌘R already use, so native NSTextField/TextEditor
