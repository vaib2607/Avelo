...
Every type, method, table, and column name in Mally is locked here. Later passes must use these exact names. Renaming requires updating this document first. Ad-hoc renames are rejected.
...
| Symbol | Kind | File |
|---|---|---|
| `MallyApp` | `struct` (App protocol) | `App/MallyApp.swift` |
...
Tables (all under `mally_` prefix to keep namespacing obvious if we ever attach another DB):
```
mally_companies
...
Registry DB tables (`mally_registry.sqlite`):
```
mally_registry_companies
```

Full column list lives in `Mally_Schema.md`. Do not deviate from those column names.
