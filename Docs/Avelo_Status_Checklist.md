...
- [x] UI audit #9b Balance Sheet scoped-read repair: `ReportService.balanceSheet` validates selected company/FY/as-of before its read transaction, verifies selected-FY integrity before as-of activity, reconciles only selected-FY activity, then caches only a complete candidate. `ReportsViewModel` validates both primary/comparative scopes before either candidate read, publishes candidates atomically, and resets same-company FY scope without recreating the model; the local panel identifies primary/comparison scope and never renders a nil result as empty success. `BalanceSheetReconciliationTests` 10/0 and `ReportsViewModelTests` 14/0 passed on the current worktree; bundle validation/self-test/launch smoke and bundled macOS keyboard/VoiceOver/visual/accountant acceptance were confirmed on 2026-07-19.
