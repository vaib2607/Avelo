...
- `Mally_Master_PRD.md` — fields, screens, error strings, shortcuts.
- `Mally_Architecture.md` — layers, MVVM, `SQLiteDatabase` API.
- `Mally_Rules.md` — non-negotiables (offline, paise, double-entry, FY locks, audit immutability, reversal-not-delete).
- `Mally_Schema.md` — frozen SQLite schema.
- `Mally_Module_Checklist.md` — per-file completion tracker.
- `Mally_Naming_Freeze.md` — authoritative type / method / table / column names.
...
- `MallyApp.swift` — `@main`. Sets up commands, env, hosts `RootView`.
...
- `VoucherSequenceRepository.swift` — atomic per-(company, FY, type) sequence using `mally_voucher_sequences`.
...
- `RegistryRepository.swift` — points at `mally_registry.sqlite`.
...
- `FiscalLockChecker.swift` — `isLocked(financialYearId:)` reads `mally_financial_years.is_locked`.
...
- No file under `Mally/` imports anything from the network stack (`URLSession`, `Network.framework`, etc.) and no `Process` / `Shell` calls exist anywhere in the project.
