...
public struct RegistryRepository: Sendable {
    ...
    public func listCompanies() throws -> [CompanyRegistryEntry] {
        try db.query(
            "SELECT id, name, sqlite_file_name, last_opened_at, created_at FROM mally_registry_companies ORDER BY name COLLATE NOCASE"
        ) { r in
    ...
    public func register(_ entry: CompanyRegistryEntry) throws {
        try db.execute(
            """
            INSERT OR REPLACE INTO mally_registry_companies
    ...
    public func unregister(id: Company.ID) throws {
        try db.execute(
            "DELETE FROM mally_registry_companies WHERE id = ?",
            [.text(id.uuidString)]
        )
    }
