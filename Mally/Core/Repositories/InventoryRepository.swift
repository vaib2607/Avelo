...
public struct InventoryRepository: Sendable {
    ...
    public func findItemById(_ id: InventoryItem.ID) throws -> InventoryItem? {
        try db.queryOne(
            "SELECT id, company_id, code, name, unit, valuation_method, is_active, created_at FROM mally_inventory_items WHERE id = ?",
    ...
    public func findItemByCode(_ code: String, companyId: Company.ID) throws -> InventoryItem? {
        try db.queryOne(
            "SELECT id, company_id, code, name, unit, valuation_method, is_active, created_at FROM mally_inventory_items WHERE company_id = ? AND code = ?",
    ...
    public func listItemsForCompany(_ companyId: Company.ID, includeInactive: Bool = false) throws -> [InventoryItem] {
        let sql = "SELECT id, company_id, code, name, unit, valuation_method, is_active, created_at FROM mally_inventory_items WHERE company_id = ?\(includeInactive ? "" : " AND is_active = 1") ORDER BY code COLLATE NOCASE"
    ...
    public func insertItem(_ item: InventoryItem) throws {
        try db.execute(
            "INSERT INTO mally_inventory_items (id, company_id, code, name, unit, valuation_method, is_active, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            [
    ...
    public func updateItem(_ item: InventoryItem) throws {
        try db.execute(
            "UPDATE mally_inventory_items SET code = ?, name = ?, unit = ?, valuation_method = ?, is_active = ? WHERE id = ?",
            [
                .text(item.code),
                .text(item.name),
                .text(item.unit),
    ...
    public func runningBalance(itemId: InventoryItem.ID, asOf: Date) throws -> ItemBalance {
        let asOfStr = DateFormatters.isoDate(asOf)
        let row: (Int64, Int64, Int64, Int64, Int64)? = try db.queryOne(
            """
            SELECT
