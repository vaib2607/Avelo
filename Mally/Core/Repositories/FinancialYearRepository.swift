...
public struct FinancialYearRepository: Sendable {
    ...
    public func findById(_ id: FinancialYear.ID) throws -> FinancialYear? {
        try db.queryOne(
            "SELECT id, company_id, label, start_date, end_date, books_begin_date, is_locked, is_closed, created_at FROM mally_financial_years WHERE id = ?",
    ...
    public func listForCompany(_ companyId: Company.ID) throws -> [FinancialYear] {
        try db.query(
            "SELECT id, company_id, label, start_date, end_date, books_begin_date, is_locked, is_closed, created_at FROM mally_financial_years WHERE company_id = ? ORDER BY start_date ASC",
    ...
    public func findOpenForCompany(_ companyId: Company.ID) throws -> [FinancialYear] {
        try db.query(
            "SELECT id, company_id, label, start_date, end_date, books_begin_date, is_locked, is_closed, created_at FROM mally_financial_years WHERE company_id = ? AND is_locked = 0 ORDER BY start_date ASC",
            bind: [.text(companyId.uuidString)]
        ) { try Self.rowToFinancialYear($0) }
    }
    ...
    public func findMostRecent(_ companyId: Company.ID) throws -> FinancialYear? {
        try db.queryOne(
            "SELECT id, company_id, label, start_date, end_date, books_begin_date, is_locked, is_closed, created_at FROM mally_financial_years WHERE company_id = ? ORDER BY start_date DESC LIMIT 1",
            bind: [.text(companyId.uuidString)]
        ) { try Self.rowToFinancialYear($0) }
    }
    ...
    public func insert(_ fy: FinancialYear) throws {
        try db.execute(
            "INSERT INTO mally_financial_years (id, company_id, label, start_date, end_date, books_begin_date, is_locked, is_closed, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
    ...
    public func lock(_ id: FinancialYear.ID) throws {
        try db.execute(
            "UPDATE mally_financial_years SET is_locked = 1 WHERE id = ?",
            [.text(id.uuidString)]
        )
    }
    ...
    public func unlock(_ id: FinancialYear.ID) throws {
        try db.execute(
            "UPDATE mally_financial_years SET is_locked = 0 WHERE id = ?",
            [.text(id.uuidString)]
        )
    }
    ...
    public func markClosed(_ id: FinancialYear.ID) throws {
        try db.execute(
            "UPDATE mally_financial_years SET is_closed = 1, is_locked = 1 WHERE id = ?",
            [.text(id.uuidString)]
        )
    }
