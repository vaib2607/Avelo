...
public struct VoucherNumberGenerator: Sendable {
    ...
    public func next(companyId: Company.ID,
        ...
        let row = try db.queryOne(
            "SELECT prefix, padding, last_number FROM mally_voucher_sequences WHERE company_id = ? AND financial_year_id = ? AND voucher_type_code = ?",
            bind: [
        ...
        if let row = row {
            ...
            let nextN = row.2 + 1
            try db.execute(
                "UPDATE mally_voucher_sequences SET last_number = ? WHERE company_id = ? AND financial_year_id = ? AND voucher_type_code = ?",
                [
                    .integer(Int64(nextN)),
                    .text(companyId.uuidString),
                    .text(financialYearId.uuidString),
            ...
            try db.execute(
                "INSERT INTO mally_voucher_sequences (company_id, financial_year_id, voucher_type_code, last_number, prefix, suffix, padding) VALUES (?, ?, ?, 1, ?, NULL, ?)",
                [
                    .text(companyId.uuidString),
                    .text(financialYearId.uuidString),
                    .text(typeCode.rawValue),
    ...
    private func shortFY(of fyId: FinancialYear.ID) throws -> String {
        let label: String? = try db.queryOne(
            "SELECT label FROM mally_financial_years WHERE id = ?",
