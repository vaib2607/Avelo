    ...
    public func loadDefaults(into db: SQLiteDatabase,
        ...
        try db.write { tx in
            for vt in payload.voucherTypes {
                try tx.execute(
                    "INSERT INTO mally_voucher_types (id, company_id, code, name, abbreviation, is_system, affects_inventory, sort_order, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [
            ...
            for g in payload.groups {
                try tx.execute(
                    "INSERT INTO mally_account_groups (id, company_id, parent_group_id, code, name, nature, is_active, sort_order, created_at) VALUES (?, ?, NULL, ?, ?, ?, 1, ?, ?)",
                    [
                        .text(UUID().uuidString),
                        .text(companyId.uuidString),
                        .text(g.code),
            ...
            let groups = try tx.query("SELECT id, code FROM mally_account_groups WHERE company_id = ?",
            ...
            for l in payload.ledgers {
                ...
                try tx.execute(
                    "INSERT INTO mally_accounts (id, company_id, group_id, code, name, opening_balance_paise, opening_balance_side, is_active, is_bank_account, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)",
                    [
                        .text(UUID().uuidString),
                        .text(companyId.uuidString),
                        .text(gid),
            ...
            for vt in VoucherType.Code.allCases {
                try tx.execute(
                    "INSERT OR REPLACE INTO mally_voucher_sequences (company_id, financial_year_id, voucher_type_code, last_number, prefix, suffix, padding) VALUES (?, ?, ?, 0, ?, NULL, ?)",
                    [
                        .text(companyId.uuidString),
